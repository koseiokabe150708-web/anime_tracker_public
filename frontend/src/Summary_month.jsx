...
function Summary_month() {
    ...
    const monthOptions = [...new Set(animeList.map((anime) => new Date(anime.anime_date).getMonth() + 1))].sort((a, b) => {
  if (a === "all") return -1;
  return a - b;
})
    ...
    const rows = monthOptions.map((m) => {
        const entries = animeList.filter((anime) => new Date(anime.anime_date).getMonth() + 1 === m)
        ...
        const character = animeList.filter((anime) => new Date(anime.anime_date).getMonth() + 1 === m && anime.character_appear === true)
        const wins = animeList.filter((anime) => new Date(anime.anime_date).getMonth() + 1 === m && anime.positive_score > anime.negative_score)
        const loses = animeList.filter((anime) => new Date(anime.anime_date).getMonth() + 1 === m && anime.positive_score < anime.negative_score)
        return { id: m, month: m, positive: pos, negative: neg, total: pos - neg, win: wins.length, lose: loses.length, episodes: entries.length, episodes_character: character.length, 
...
return (
    <Layout>
            ...
            <h1 style={{ color: "#1a1a1a" }}>Summary Month</h1>
